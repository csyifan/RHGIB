...
class basehgnn(nn.Module):
    def __init__(self, num_class, input_dimensions, embeddings_dimension=64, num_gnns=2, dropout=0, temper=1.0, num_type=4, beta=1):

        super(basehgnn, self).__init__()
        ...
        for layer in range(self.num_gnns):
            self.GCNLayers.append(GraphConv(
                self.embeddings_dimension, self.embeddings_dimension, weight=True, activation=F.relu))
    ...
    def forward(self, g, features_list, seqs, type_emb, node_type, norm=False, edge_weight = None):
        h = []
        ...
        for layer in range(self.num_gnns):
            if edge_weight is not None:
                gh = self.GCNLayers[layer](g, gh, edge_weight = edge_weight)
            else:
                gh = self.GCNLayers[layer](g, gh)
            gh = self.Drop(gh)
            r = self.RELayers[layer](g, r, node_type)
        h = gh[seqs]
        r = r[seqs]
        for layer in range(self.num_layers):
